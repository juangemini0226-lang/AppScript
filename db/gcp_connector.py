...
class GCPCloudSQLConnector(DBConnector):
    ...
    def connect(self) -> None:
        ...
        credentials = None
        if creds_json:
            # Streamlit Cloud / entorno sin ADC: se construyen credenciales
            # explícitas a partir del JSON de la Service Account, en vez de
            # depender de Application Default Credentials (que en GCE/GCR
            # intentan el metadata server, y fallan fuera de GCP).
        ...
        self._connector = Connector(ip_type=ip_type, credentials=credentials)
